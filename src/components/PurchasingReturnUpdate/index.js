...
@Form.create()
 class Purchasereturn extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    //     categoryData: nextProps.product.categoryData,
  ...
  saleGoods = () => {
    ...
    console.log(payload);
    this.props.dispatch({
      type: 'openbill/updatePurchReturn',
      payload,
      callback: (res) => {
        if (res.errcode) {
  ...
  addSaleProduct = value => {
    if (value === '') return;
    const ProductDataProps = this.props.product.productData;
    const reg = new RegExp(value, 'gi');
    const searchData = ProductDataProps.map(record => {
      const match = record.bar_code.match(reg);
      if (!match) {
        return null;
      }
      return { ...record };
